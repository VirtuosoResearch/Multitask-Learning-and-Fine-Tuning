...
def meta_train(model, saver, sess, exp_string, data_generator, resume_itr=0):
	...
	for itr in range(resume_itr, FLAGS.meta_train_iterations):
		...
		input_batch, label_batch = data_generator.sample_batch("meta_train", batch_size=FLAGS.meta_batch_size)
		inputa = input_batch[:, :, :FLAGS.k_shot, :]
		inputb = input_batch[:, :, FLAGS.k_shot:, :]
		labela = label_batch[:, :, :FLAGS.k_shot, :]
		labelb = label_batch[:, :, FLAGS.k_shot:, :]
		#############################
		...
		if (itr!=0) and itr % TEST_PRINT_INTERVAL == 0:
		    ...
		    # group a (inputa, labela) and group b (inputb, labelb)
			input_batch, label_batch = data_generator.sample_batch("meta_val", batch_size=FLAGS.meta_batch_size)
			inputa = input_batch[:, :, :FLAGS.k_shot, :]
			inputb = input_batch[:, :, FLAGS.k_shot:, :]
			labela = label_batch[:, :, :FLAGS.k_shot, :]
			labelb = label_batch[:, :, FLAGS.k_shot:, :]
...
def meta_test(model, saver, sess, exp_string, data_generator, meta_test_num_inner_updates=None):
	...
	for _ in range(NUM_META_TEST_POINTS):
		...
		# group a (inputa, labela) and group b (inputb, labelb)
		input_batch, label_batch = data_generator.sample_batch("meta_test", batch_size=FLAGS.meta_batch_size)
		inputa = input_batch[:, :, :FLAGS.k_shot, :]
		inputb = input_batch[:, :, FLAGS.k_shot:, :]
		labela = label_batch[:, :, :FLAGS.k_shot, :]
		labelb = label_batch[:, :, FLAGS.k_shot:, :]
