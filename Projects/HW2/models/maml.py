	...
	def construct_model(self, prefix='maml'):
		...
		with tf.variable_scope('model', reuse=None) as training_scope:
			...
			def task_inner_loop(inp, reuse=True):
				...
				if FLAGS.stop_grad:
					grads = [tf.stop_gradient(grad) for grad in grads] # stop the second derivative
				...
				for j in range(num_inner_updates-1):
					...
					if FLAGS.stop_grad:
						grads = [tf.stop_gradient(grad) for grad in grads] # stop the second derivative
					gradients = dict(zip(weights.keys(), grads))
					fast_weights = dict(zip(weights.keys(), [weights[key] - self.meta_lr*gradients[key] for key in weights.keys()]))
					output = self.forward(inputb, fast_weights, reuse=True)
					task_outputbs.append(output)
					task_lossesb.append(self.loss_func(output, labelb))
